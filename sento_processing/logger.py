...
from sento_processing import settings
...
def _get_logging_settings():
    ...
    config_lvl = config.LOGGING_LEVEL
    config_out_dst = config.LOGGING_OUTPUT

    lvl = logging.getLevelName(config_lvl)
    out_dst = None  # type: str

    if config_out_dst in VALID_OUTPUTS:
        out_dst = config_out_dst
    else:
